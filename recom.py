...
import random
class itemCF:
    def __init__(self, dl, sm, ratingDict):
    ...
    def getRatingRank(self, rating_dict):
        ratingRank = sorted(list(self.ratingDict.items()), key=lambda x:x[1], reverse=True)
        return ratingRank
    ...
    def recommend(self, top, rating_dict, shuffle=False):
        ...
        for item in self.ratingRank:
            rec_item_num_dict[item[0]] = int(item[1] / total_rate * top)
        ...
        for item in self.ratingRank:
            ...
            idx_rank = np.argsort(-self.sim_mat[idx])[:topn]
